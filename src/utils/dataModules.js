// 活动数据 / 单品
export const activityItemData = [
    {
        prop: 'brand',
        label: '品牌',
        width: 120
    },
    {
        prop: 'qudao',
        label: '渠道',
        width: 120
    },
    {
        prop: 'shop',
        label: '店铺',
        width: 120
    },
    {
        prop: 'huodongCreatTime',
        label: '活动开始时间',
        width: 150
    },
    {
        prop: 'huodongEndTime',
        label: '活动结束时间',
        width: 150
    },
    {
        prop: 'huodongPreheatTime',
        label: '活动预热时间',
        width: 150
    },
    {
        prop: 'huodongProtectTime',
        label: '活动保护期',
        width: 150
    },
    {
        prop: 'huodongName',
        label: '活动名称',
        width: 120
    },
    {
        prop: 'hdDengji',
        label: '活动力度（等级）',
        width: 120
    },
    {
        prop: 'itemSpu',
        label: 'spu',
        width: 120
    },
    {
        prop: 'itemSku',
        label: 'sku',
        width: 120
    },
    {
        prop: 'huodongPrice',
        label: '活动价格',
        width: 120
    },
    {
        prop: 'participateInActivityInventory',
        label: '参与活动库存量',
        width: 150
    },
]

// 活动数据 / 店铺
export const activityStoreData = [
    {
        prop: 'brand',
        label: '品牌',
        width: 120
    },
    {
        prop: 'qudao',
        label: '渠道',
        width: 120
    },
    {
        prop: 'shop',
        label: '店铺',
        width: 120
    },
    {
        prop: 'huodongCreatTime',
        label: '活动开始时间',
        width: 150
    },
    {
        prop: 'huodongEndTime',
        label: '活动结束时间',
        width: 150
    },
    {
        prop: 'huodongPreheatTime',
        label: '活动预热时间',
        width: 150
    },
    {
        prop: 'huodongProtectTime',
        label: '活动保护期',
        width: 150
    },
    {
        prop: 'huodongName',
        label: '活动名称',
        width: 120
    },
    {
        prop: 'hdDengji',
        label: '活动力度（等级）',
        width: 120
    },
    {
        prop: 'huodongTarget',
        label: '活动目标',
        width: 120
    },
    {
        prop: 'huodongBenefit',
        label: '活动利益点',
        width: 120
    },
    {
        prop: 'salesTarget',
        label: '销售目标',
        width: 120
    },
    {
        prop: 'zhekouTarget',
        label: '折扣目标（利润率目标）',
        width: 150
    },
    {
        prop: 'targetLiuliang',
        label: '目标流量',
        width: 120
    },
    {
        prop: 'targetKedan',
        label: '目标客单',
        width: 120
    },
    {
        prop: 'targetZhuanhua',
        label: '目标转化率',
        width: 120
    },
    {
        prop: 'targetLiandan',
        label: '目标连单率',
        width: 120
    },
]

// 竞争数据
export const competeData = [
    {
        prop: 'dateTime',
        label: '日期'
    },
    {
        prop: 'jingpinShop',
        label: '竞品商铺'
    },
    {
        prop: 'contendSpu',
        label: 'spu'
    },
    {
        prop: 'contendSku',
        label: 'SKU'
    },
    {
        prop: 'salesVolume',
        label: '销售量'
    },
    {
        prop: 'collect',
        label: '收藏'
    },
    {
        prop: 'price',
        label: '价格'
    },
    {
        prop: 'xiaoshouMoney',
        label: '销售金额'
    },
    {
        prop: 'huodongName',
        label: '活动名称'
    },
]

// 商品销售数据
export const GoodsData = [
    {
        prop: 'ngjiDate',
        label: '统计时间',
        width: 120
    },
    {
        prop: 'commodityId',
        label: '商品id',
        width: 120
    },
    {
        prop: 'commodityName',
        label: '商品名称',
        width: 120
    },
    {
        prop: 'merchandiseTraffic',
        label: '商品浏览量',
        width: 120
    },
    {
        prop: 'visitorNumber',
        label: '访客数',
        width: 120
    },
    {
        prop: 'xiangqing',
        label: '详情跳出率',
        width: 120
    },
    {
        prop: 'tingliuAve',
        label: '平均停留时长',
        width: 120
    },
    {
        prop: 'jiagouNumber',
        label: '加购件数',
        width: 120
    },
     {
        prop: 'skuJiagouNumber',
        label: 'sku加购件数',
        width: 120
    },
    {
        prop: 'shoucangNumber',
        label: '收藏人数',
        width: 120
    },
    {
        prop: 'zfmjNumber',
        label: '支付买家数',
        width: 120
    },
    {
        prop: 'zhifuNumber',
        label: '支付件数',
        width: 120
    },
    {
        prop: 'zhifuMoney',
        label: '支付金额',
        width: 120
    },
    {
        prop: 'zhifuLv',
        label: '支付转化率',
        width: 120
    },
    {
        prop: 'xiadanZhifuLv',
        label: '下单支付转化率',
        width: 120
    },
    {
        prop: 'xiadanBuyNumber',
        label: '下单买家数',
        width: 120
    },
    {
        prop: 'xiadanNumber',
        label: '下单件数',
        width: 120
    },
    {
        prop: 'xiadanMoney',
        label: '下单金额',
        width: 120
    },
    {
        prop: 'xiadanLv',
        label: '下单转化率',
        width: 120
    },
    {
        prop: 'sousuoVisitor',
        label: '搜索引导访客数',
        width: 120
    }, 
    {
        prop: 'sousuoZhifuLv',
        label: '搜索支付转化率',
        width: 120
    }, 
    {
        prop: 'ssYdBn',
        label: '搜索引导支付买家数',
        width: 150
    }, 
    {
        prop: 'describes',
        label: '描述相符',
        width: 120
    }, 
    {
        prop: 'fmPj',
        label: '负面评价',
        width: 120
    }, 
    {
        prop: 'top',
        label: '地域top5',
        width: 120
    },
]

// 生意参谋市场行情行业大盘数据
export const industryData = [
    {
        prop: 'dateTime',
        label: '日期',
        width: 120
    },
    {
        prop: 'type',
        label: '类目',
        width: 120
    },
    {
        prop: 'numberOfVisitors',
        label: '访客数',
        width: 120
    },
    {
        prop: 'liulanContent',
        label: '浏览量',
        width: 120
    },
    {
        prop: 'numberOfClicks',
        label: '搜索点击人数',
        width: 120
    },
    {
        prop: 'numberOfSearchClicks',
        label: '搜索点击次数',
        width: 120
    },
    {
        prop: 'searchRate',
        label: '搜索点击率',
        width: 120
    },
    {
        prop: 'shoucangPeopleNumber',
        label: '收藏人数',
        width: 120
    },
    {
        prop: 'scContent',
        label: '收藏次数',
        width: 120
    },
    {
        prop: 'jiagouPeopleNumber',
        label: '加购人数',
        width: 120
    },
    {
        prop: 'shoucangContent',
        label: '加购次数',
        width: 120
    },
    {
        prop: 'customerTransactionPrice',
        label: '客单价',
        width: 120
    },
    {
        prop: 'liulanShopContent',
        label: '浏览商品数',
        width: 120
    },
    {
        prop: 'numberOfSellers',
        label: '卖家数',
        width: 120
    },
    {
        prop: 'beiliulanMaijiaContent',
        label: '被浏览卖家数',
        width: 120
    },
    {
        prop: 'beizhifuMaijiaContent',
        label: '被支付卖家数',
        width: 120
    },
    {
        prop: 'zhifuContent',
        label: '支付件数',
        width: 120
    },
    {
        prop: 'searchPopularity',
        label: '搜索人气',
        width: 120
    },
    {
        prop: 'tradingIndex',
        label: '交易指数',
        width: 120
    },
    {
        prop: 'zhifuJiaoflZhanbi',
        label: '支付金额较父类目占比',
        width: 150
    },
]

// 运营动作记录数据 / 单品
export const operateItemData = [
    {
        prop: 'tongjiTime',
        label: '统计时间'
    },
    {
        prop: 'qudao',
        label: '渠道'
    },
    {
        prop: 'shop',
        label: '店铺'
    },
    {
        prop: 'wayOfPromotion',
        label: '推广方式'
    },
    {
        prop: 'itemSpu',
        label: 'spu'
    }
]

// 运营动作记录数据 / 店铺
export const operateStoreData = [
    {
        prop: 'tongjiTime',
        label: '统计时间'
    },
    {
        prop: 'qudao',
        label: '渠道'
    },
    {
        prop: 'shop',
        label: '店铺'
    },
    {
        prop: 'wayOfPromotion',
        label: '推广方式'
    },
    {
        prop: 'activityInterest',
        label: '活动利益点'
    },
    {
        prop: 'salesTarget',
        label: '销售目标'
    },
    {
        prop: 'targetLiuliang',
        label: '目标流量'
    },
    {
        prop: 'targetKedan',
        label: '目标客单'
    },
    {
        prop: 'targetZhuanhua',
        label: '目标转化率'
    },
    {
        prop: 'targetLiandan',
        label: '目标连单率'
    },
]

// 销售目标数据
export const salesData = [
    {
        prop: 'tongjiTime',
        label: '统计时间'
    },
    {
        prop: 'brand',
        label: '品牌'
    },
    {
        prop: 'qudao',
        label: '渠道'
    },
    {
        prop: 'shop',
        label: '店铺'
    },
    {
        prop: 'salesETarget',
        label: '销售额目标'
    },
    {
        prop: 'salesLiangTarget',
        label: '销售量目标'
    },
    {
        prop: 'discountTarget',
        label: '折扣（毛利率）目标'
    },
    {
        prop: 'profitTarget',
        label: '利润额目标'
    },
    {
        prop: 'trafficTarget',
        label: '流量目标'
    },
    {
        prop: 'guestSingleObjective',
        label: '客单目标'
    },
    {
        prop: 'conversionTarget',
        label: '转化率目标'
    },
]

// 店铺数据
export const shopData = [
    {
        prop: 'dateTime',
        label: '日期'
    },
    {
        prop: 'jingpinShop',
        label: '竞品商铺'
    },
    {
        prop: 'contendSpu',
        label: 'spu'
    },
    {
        prop: 'contendSku',
        label: 'SKU'
    },
    {
        prop: 'collect',
        label: '收藏'
    },
    {
        prop: 'price',
        label: '价格'
    },
    {
        prop: 'xiaoshouMoney',
        label: '销售金额'
    },
    {
        prop: 'huodongName',
        label: '活动名称'
    },
]